<template>
  ...
  <v-card>
      ...
      <div class="d-flex flex-row">
        ...
        <v-window v-model="tab">
          ...
          <v-window-item value="feature-flags">
            <v-card flat>
            <v-card-text>
                <p class="mb-4">
                  Enable or disable features. Changes are saved to your browser.
                </p>
                <div v-for="flag in featureFlagStore.allFlags" :key="flag.key" class="mb-2">
                  <v-switch
                    :model-value="flag.enabled"
                    @update:model-value="(v) => featureFlagStore.setFlag(flag.key, v)"
                    :label="flag.label"
  ...
  function updateMainnetRPC(rpc){
    chainStore.updateProvider(0, rpc);
  }
  function updateArbitrumRPC(rpc){
    chainStore.updateProvider(1, rpc);
  }
