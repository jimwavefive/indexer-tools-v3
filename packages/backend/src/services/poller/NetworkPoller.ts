...
import { createGraphClient, GET_ALLOCATIONS, GET_GRAPH_NETWORK } from '@indexer-tools/shared';
...
export class NetworkPoller {
  ...
  constructor() {
    ...
    this.client = createGraphClient(this.networkSubgraphUrl);
  }
  ...
  async fetchAllocations(indexerAddress: string): Promise<Allocation[]> {
    ...
    while (true) {
      const data = await this.client.request<{ allocations: Allocation[] }>(
        GET_ALLOCATIONS,
        { indexer: indexerAddress, skip },
      );
  ...
  async fetchIndexerData(indexerAddress: string): Promise<IndexerData | null> {
    ...
    const data = await this.client.request<{
      indexer?: { tokenCapacity: string; stakedTokens: string; allocatedTokens: string; availableStake: string };
    }>(query);
