...
import type { RuleConfig, ChannelConfig, IncidentRecord, HistoryRecord } from '@indexer-tools/shared';
...
const DEFAULT_RULES: RuleConfig[] = [
  ...
  {
    id: 'failed-subgraph-stale',
    name: 'Failed Subgraph (Stale)',
    type: 'failed_subgraph_stale',
    enabled: true,
    conditions: { minGrt: 10000, allowedActions: [] },
  },
  {
    id: 'failed-subgraph-deterministic',
    name: 'Failed Subgraph (Deterministic)',
    type: 'failed_subgraph_deterministic',
    enabled: true,
    conditions: { minGrt: 10000, allowedActions: [] },
  },
  {
    id: 'failed-subgraph-nondeterministic',
    name: 'Failed Subgraph (Non-Deterministic)',
    type: 'failed_subgraph_nondeterministic',
    enabled: true,
    conditions: { minGrt: 10000, allowedActions: [] },
  },
  {
...
export class SqliteStore {
  ...
  private initSchema(): void {
    ...
    const migrations = [
      ...
      'ALTER TABLE history ADD COLUMN is_test INTEGER NOT NULL DEFAULT 0',
      'ALTER TABLE rules ADD COLUMN cooldown_minutes INTEGER',
      'ALTER TABLE rules ADD COLUMN polling_interval_seconds INTEGER',
    ...
    // Migrate polling_interval_seconds → polling_interval_minutes
    try {
      this.db.exec(
        'UPDATE rules SET polling_interval_minutes = polling_interval_seconds / 60 WHERE polling_interval_seconds IS NOT NULL AND polling_interval_minutes IS NULL',
      );
    } catch { /* ignore */ }

    // Seed default settings
    this.db.prepare('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)').run(
      ...
      const migrate = this.db.transaction(() => {
        ...
        if (data.history) {
          for (const h of data.history) {
            insertHistory.run(
              h.id, null, h.notification?.ruleId || '', h.notification?.title || '',
              h.notification?.message || '', h.notification?.severity || 'info',
              h.notification?.timestamp || h.timestamp, JSON.stringify(h.channelIds || []),
              JSON.stringify(h.notification?.metadata || {}), h.timestamp,
            );
          }
        }
      });
  ...
  async getChannels(): Promise<ChannelConfig[]> {
    ...
    return rows.map((c) => ({
      id: c.id, name: c.name, type: c.type, enabled: c.enabled === 1,
  ...
  async getHistory(): Promise<HistoryRecord[]> {
    const rows = this.db.prepare('SELECT * FROM history ORDER BY created_at DESC').all() as Array<{
    ...
    return rows.map((h) => ({
      id: h.id, incidentId: h.incident_id || undefined,
      notification: {
        title: h.title, message: h.message,
        severity: h.severity as 'info' | 'warning' | 'critical',
        timestamp: h.notification_timestamp, ruleId: h.rule_id,
        metadata: JSON.parse(h.metadata),
      },
      channelIds: JSON.parse(h.channel_ids), timestamp: h.created_at,
      isTest: h.is_test === 1,
    }));
  }
  ...
  async addHistory(record: HistoryRecord): Promise<void> {
    this.db.prepare(
      ...
      record.id, record.incidentId || null, record.notification?.ruleId || '',
      record.notification?.title || '', record.notification?.message || '',
      record.notification?.severity || 'info', record.notification?.timestamp || record.timestamp,
      JSON.stringify(record.channelIds || []), JSON.stringify(record.notification?.metadata || {}),
      record.timestamp, record.isTest ? 1 : 0,
    );
  }
  ...
  getActiveIncident(ruleId: string, targetKey: string): IncidentRecord | null {
    ...
    if (!row) return null;
    return { ...row, latest_metadata: JSON.parse(row.latest_metadata), channel_ids: JSON.parse(row.channel_ids) };
  }
  ...
  createIncident(incident: IncidentRecord): void {
    this.db.prepare(
      ...
      incident.id, incident.rule_id, incident.target_key, incident.target_label,
      incident.severity, incident.status, incident.auto_resolve,
      incident.first_seen, incident.last_seen, incident.last_notified_at || incident.first_seen,
      incident.resolved_at, incident.occurrence_count, incident.latest_title, incident.latest_message,
      JSON.stringify(incident.latest_metadata), JSON.stringify(incident.channel_ids),
    );
  }
  ...
  getIncidents(options?: { status?: string; limit?: number; offset?: number }): IncidentRecord[] {
    ...
    return rows.map((r) => ({ ...r, latest_metadata: JSON.parse(r.latest_metadata), channel_ids: JSON.parse(r.channel_ids) }));
  }
  ...
  getIncidentById(id: string): (IncidentRecord & { history: HistoryRecord[] }) | null {
    ...
    const incident: IncidentRecord = { ...row, latest_metadata: JSON.parse(row.latest_metadata), channel_ids: JSON.parse(row.channel_ids) };
    const historyRows = this.db.prepare('SELECT * FROM history WHERE incident_id = ? ORDER BY created_at DESC').all(id) as any[];
  ...
  autoResolveIncidents(firedKeys: Set<string>, enabledRuleIds?: Set<string>): { count: number; resolvedIds: string[] } {
    ...
    const update = this.db.prepare("UPDATE incidents SET status = 'resolved', resolved_at = ? WHERE id = ?");
